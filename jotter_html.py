...
def create_static_index(filename_map, citekey_map, keyword_map):
    """
...
def produce_html(doc, css_root, citekey_map=None, internal_links=False,
        resolve_citekeys=True) -> str:
    # TODO: This css stuff currently breaks everything :p
    pandoc_args = [
            "--template", css_root + "/tufte.html5",
            #"--css", css_root + "/tufte.css",
            #"--css", css_root + "/pandoc.css",
            #"--css", css_root + "/pandoc-solarized.css",
            #"--css", css_root + "/jotter.css",
            "-F", "pandoc-sidenote",
            "-F", "pandoc-static-katex",
    ]
    ...
    if "_metadata_codeblock" in doc.keys():
        content = ":::fullwidth\n{}\n:::\n\n{}".format(
                doc["_metadata_codeblock"],
                content,
            )
    ...
    if citekey_map != None:
        html = jotter_citeproc(html, doc=doc, citekey_map=citekey_map,
            resolve_citekeys=resolve_citekeys)

    return html

def jotter_citeproc(html: str, doc: dict, citekey_map: dict,
        resolve_citekeys: bool) -> str:
    ...
    for citation in html.find_all(attrs={"class": "citation"}):
        ...
        for key in citation.get("data-cites").split():
            ...
            content.append(link(oldkey, prefix, resolve_citekeys))
            oldkey = key
        content.append(link(oldkey, text, resolve_citekeys))
...
if __name__ == "__main__":
    ...
    index = {
        "_content": create_static_index(
            filename_map, citekey_map, keyword_map
        ),
        "_html_filename": "index.html",
        "title": "Jotter Index",
    }
    with open(os.path.join(html_root, index["_html_filename"]), "w") as f:
        f.write(produce_html(
            index,
            css_root=jotter_root + "/.jotter/css",
            internal_links=True,
            resolve_citekeys=True,
        ))
    ...
    for filename, doc in filename_map.items():
        ...
        if needs_update(doc, html_root=html_root):
            outfile = os.path.join(html_root, doc["_html_filename"])
